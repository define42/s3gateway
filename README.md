...
- Supports legacy and encrypted credential formats for `AccessKey`:
  ...
  - Encrypted: `GenerateKeysX25519(...)` output (`X1...`)
...
- `AWS_ACCESS_KEY_ID` supports:
  ...
  - `GenerateKeysX25519(...)` access key output (`X1...`, encrypted mode)
...
- SigV4 request verification still uses `SIGV4_SECRET` in both legacy and encrypted modes.
...
- Keep `AWS_SECRET_ACCESS_KEY` set to the configured `SIGV4_SECRET` for request signing.
