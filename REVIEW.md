...
2. **Admin-UI upload** (`adminpage.go:1525-1531`): `uploaded-by` is stamped from the session (good), but required metadata keys are never checked, so the browser upload path silently bypasses the policy that the S3 API rejects with 400. *(Still open.)*

**Recommendation:** ~~run `missingRequiredUploadMetadata` + `ensureUploadedByMetadata` on the copy path when the directive is `REPLACE`~~ (done); for the admin path, either enforce required keys on uploads or let the form collect them. Note the `COPY` directive still inherits the source object's metadata verbatim, so a non-compliant source (created before the policy, via the admin path, or written directly to the backend) can still propagate — closing that would require HEAD-ing the source, tracked separately.
