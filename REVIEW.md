...
1. **CopyObject / UploadPartCopy** (`handlers_object.go:1323`): with `x-amz-metadata-directive: REPLACE` the caller fully controls the destination metadata — required keys are not checked and, unlike PutObject, `uploaded-by` is **not** overwritten, so it can be spoofed or omitted. Any user with `w` on a destination and `r` on any source can mint objects that violate the metadata policy.
2. **Admin-UI upload** (`adminpage.go:1525-1531`): `uploaded-by` is stamped from the session (good), but required metadata keys are never checked, so the browser upload path silently bypasses the policy that the S3 API rejects with 400.

**Recommendation:** run `missingRequiredUploadMetadata` + `ensureUploadedByMetadata` on the copy path when the directive is `REPLACE` (for `COPY` the source object's metadata carries over), and either enforce required keys on admin uploads or let the form collect them.
